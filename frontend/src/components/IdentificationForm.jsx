...
const IdentificationForm = ({ data, onChange, onNext, onOpenSocietesModal }) => {
  ...
  const validatePeriode = (periode, regime) => {
    if (!periode || periode.trim() === '') return t('error_periode_required');
    const num = parseInt(periode, 10);
    if (isNaN(num) || num.toString() !== periode.trim() || num < 0) return t('error_periode_integer');
    if (regime === '1' && (num < 1 || num > 12)) return t('error_periode_monthly');
    if (regime === '2' && (num < 1 || num > 4)) return t('error_periode_quarterly');
    return '';
  };
  ...
  return (
    <div className="panel active" id="panel1">
      ...
      <Card>
        ...
        <div style={{ height: 1, background: 'var(--border)', margin: '16px 0 20px', transition: 'background 0.3s ease' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 16, alignItems: 'start' }}>
          ...
          {regimeDef?.label && (
              ...
              <span style={{ fontWeight: 700, color: 'var(--primary-green)' }}>{t(`regime_${regimeDef.value}`).split('—')[1]?.trim()}:</span> {regimeDef.desc || ''}
            </div>
          )}
        </div>
