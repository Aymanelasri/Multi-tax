...
const SocietesPage = () => {
  ...
  const [societes, setSocietes] = useState(load);
  const [modal, setModal] = useState(null); // null | 'add' | societe object
  const [search, setSearch] = useState('');
  const [sort, setSort] = useState('name');
  const [confirmId, setConfirmId] = useState(null);
  const [showOnboarding, setShowOnboarding] = useState(() => {
    // Show onboarding only if no companies exist AND user hasn't dismissed it
    if (load().length === 0) {
  ...
  const filtered = useMemo(() => {
    ...
    else if (sort === 'used') list = [...list].sort((a, b) => (b.usageCount || 0) - (a.usageCount || 0));
    return list;
  ...
  const handleSave = (form) => {
    let updated;
    if (modal?.id) {
      updated = societes.map(s => s.id === modal.id ? { ...s, ...form } : s);
    } else {
      updated = [...societes, { ...form, id: Date.now(), createdAt: Date.now(), usageCount: 0, lastUsed: null }];
    }
    setSocietes(updated);
    save(updated);
    setModal(null);
  ...
  const handleStartDeclContinue = () => {
    ...
    // Update usage count
    const updated = societes.map(x => x.id === startDeclModal.id ? { ...x, usageCount: (x.usageCount || 0) + 1, lastUsed: Date.now() } : x);
    setSocietes(updated);
    save(updated);
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 48px 100px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
          ...
          <button
            ...
            style={{ padding: '12px 24px', borderRadius: 10, background: 'linear-gradient(135deg,#10b981,#34d399)', border: 'none', color: '#000', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', boxShadow: '0 4px 16px rgba(16,185,129,0.3)' }}
          >
            {t('societes_add')}
          </button>
        </div>
        ...
        {/* Onboarding banner */}
        {showOnboarding && societes.length === 0 && (
        ...
        {/* No results */}
        {societes.length > 0 && filtered.length === 0 && (
