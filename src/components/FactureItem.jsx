...
const FactureItem = ({ id, data, onChange, onRemove }) => {
  ...
  return (
    <div className="facture-item">
      ...
      {/* Section: Facture */}
      <div style={sectionLabel}><span style={sectionBar}/>Facture</div>
      <div className="form-grid cols3" style={{ marginBottom: 16 }}>
        <FormGroup label="N° de Facture" required>
      ...
      {/* Section: Fournisseur */}
      <div style={sectionLabel}><span style={sectionBar}/>Fournisseur</div>
      <div className="form-grid cols3" style={{ marginBottom: 16 }}>
        <FormGroup label="IF Fournisseur" required help="1 a 8 chiffres">
          <input
            type="text" value={data.if || ''} onChange={(e) => set('if', e.target.value)}
            placeholder="ex: 33006240"
            className={data.if && !ifValid ? 'invalid' : ''}
          />
          {data.if && !ifValid && <span className="field-error">1 a 8 chiffres requis</span>}
        </FormGroup>
        <FormGroup label="Nom / Raison Sociale" required>
          <input type="text" value={data.nom || ''} onChange={(e) => set('nom', e.target.value)} placeholder="ex: ONEE" />
        </FormGroup>
        <FormGroup label="ICE" help="15 chiffres — optionnel">
          <input
      ...
      <div className="form-grid cols3">
        <FormGroup label="Mode de Paiement" required>
          <select value={data.mp || '1'} onChange={(e) => set('mp', e.target.value)}>
            {PAYMENT_MODES.map((m) => <option key={m.value} value={m.value}>{m.value} — {m.label}</option>)}
          </select>
        </FormGroup>
        <FormGroup label="Date de Paiement" required>
          <div className="date-wrap">
            <input type="date" value={data.dpai || ''} onChange={(e) => set('dpai', e.target.value)} />
          </div>
        </FormGroup>
        <FormGroup label="Date de Facture" required>
          <div className="date-wrap">
            <input type="date" value={data.dfac || ''} onChange={(e) => set('dfac', e.target.value)} />
          </div>
        </FormGroup>
        <FormGroup label="Prorata (%)" help="Optionnel — defaut 100%">
          <div className="number-wrap">
